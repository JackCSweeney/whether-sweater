...
RSpec.describe 'Request for Forecast by City' do
  ...
  describe '#sad path' do
    it 'will return a 422 error if not given a location in params' do
      get "/api/v0/forecast?location=", headers: @headers
      ...
      expect(result[:errors][:message]).to eq("Error: Unable to process request")
    end
  end
end